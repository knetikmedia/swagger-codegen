...
public class JavaClientCodegen extends AbstractJavaCodegen
	...
	protected boolean doNotUseRx = true; // backwards compatibility for swagger
											// configs that specify neither rx1
											// nor rx2 (mustache does not allow
											// for boolean operators so we need
											// this extra field)
	...
	public JavaClientCodegen() {
		...
		supportedLibraries.put("jersey1",
				"HTTP client: Jersey client 1.19.1. JSON processing: Jackson 2.7.0. Enable Java6 support using '-DsupportJava6=true'. Enable gzip request encoding using '-DuseGzipFeature=true'.");
		...
		supportedLibraries.put("okhttp-gson",
				"HTTP client: OkHttp 2.7.5. JSON processing: Gson 2.6.2. Enable Parcelable modles on Android using '-DparcelableModel=true'. Enable gzip request encoding using '-DuseGzipFeature=true'.");
		...
		supportedLibraries.put(RETROFIT_2,
				"HTTP client: OkHttp 3.2.0. JSON processing: Gson 2.6.1 (Retrofit 2.0.2). Enable the RxJava adapter using '-DuseRxJava[2]=true'. (RxJava 1.x or 2.x)");
	...
	public void processOpts() {
		...
		// put the boolean value back to PARCELABLE_MODEL in
		...
		if ("feign".equals(getLibrary())) {
			...
			// the "okhttp-gson" library template requires
			// "ApiCallback.mustache" for async call
		...
		if (Boolean.TRUE.equals(additionalProperties.get(USE_PLAY24_WS))) {
			...
			// adapters
			supportingFiles.add(
					new SupportingFile("play24/Play24CallFactory.mustache", invokerFolder, "Play24CallFactory.java"));
	...
	/**
	 * Prioritizes consumes mime-type list by moving json-vendor and json
	 * mime-types up front, but otherwise preserves original consumes definition
	 * order. [application/vnd...+json,... application/json, ..as is..]
	 * 
	 * @param consumes
	 *            consumes mime-type list
	 * @return
	 */
